...
import { GameSettings } from "./games/register-game";
import { Character } from "../systems/user/user";
import { WorldStats } from "../systems/world/world-stats";
...
export function sendChatMessage(message: string, whisperTarget?: string | null, accountType?: "bot" | "streamer") : void{
    firebot.modules.twitchChat.sendChatMessage(message, whisperTarget, accountType);
}
...
export async function updateWorldMeta(value : any, property? : string) : Promise<void> {
    const {userDb} = firebot.modules;
    const streamer = getStreamerUsername();
    if(property != null){
        await userDb.updateUserMetadata(streamer, "fbrpg-world", value, property);
    }
...
export async function getCharacterMeta(username : string) : Promise<Character> {
    const {userDb} = firebot.modules;
    const characterMeta = await userDb.getUserMetadata(username, "fbrpg-character");
    return characterMeta;
}

export async function updateCharacterMeta(userName : string, value : any, property? : string) : Promise<void> {
    const {userDb} = firebot.modules;
    await userDb.updateUserMetadata(userName, "fbrpg-character", value, property);
}
...
export function logger(type: string, message: string) : void {
    ...
    switch (type){
        case "debug":
            logger.debug("FBRPG: " + message);
        break;
        case "error":
            logger.error("FBRPG: " + message);
        break;
        case "warn":
            logger.warn("FBRPG: " + message);
        break;
        default:
            logger.info("FBRPG: " + message);
    }
}